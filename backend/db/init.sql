...
DROP TRIGGER IF EXISTS audit_log_prevent_modifications ON audit_log;
...
CREATE INDEX IF NOT EXISTS idx_conversations_citizen ON conversations(citizen_id, updated_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_grievances_citizen_status
    ON grievances(citizen_id, status);

-- Audit log: compliance and alerting range queries
CREATE INDEX IF NOT EXISTS idx_audit_log_citizen_time
